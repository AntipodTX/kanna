...
function composerStateFromSendOptions(options?: {
  ...
  if (options?.provider === "codex" && options.model && options.modelOptions?.codex) {
    return {
      ...
      model: options.model,
      modelOptions: {
        reasoningEffort: options.modelOptions.codex.reasoningEffort ?? DEFAULT_CODEX_MODEL_OPTIONS.reasoningEffort,
