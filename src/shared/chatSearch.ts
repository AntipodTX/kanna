...
export interface ProjectChatSearchEntryResult extends ChatSearchEntryResult {
  chatId: string
  chatTitle: string
  isArchived: boolean
}

export interface ProjectChatSearchCommandResult {
  query: string
  projectId: string
  matches: ProjectChatSearchEntryResult[]
  nextCursor?: string
  hasMore: boolean
}
...
export function searchTranscriptEntries(entries: TranscriptEntry[], query: string, options: ChatSearchOptions): ChatSearchEntryResult[] {
  ...
  const skipMatches = Math.max(0, options.skipMatches ?? 0)
  ...
  return entries.map((entry, index) => {
    ...
    switch (entry.kind) {
      ...
      case "context_window_updated":
        return false
      case "status":
        // Transcript rendering only shows the current tail status row.
